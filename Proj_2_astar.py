...
def check_outline(point,dimension,clearance):
    dist_check = dimension+clearance
    point_x = point[0]
    point_y = point[1]
    if point_x <= dist_check or point_x >= 250-dist_check:
        return True
    elif point_y <= dist_check or point_y >= 150-dist_check:
        return True
    else:
        return False
...
def call_obstacle_checks(point,dimension,clearance, poly_points_updated):
    if check_in_poly(point, poly_points_updated):
        ...
        return True
    elif check_outline(point,dimension,clearance):
        return True
    else:
        return False
...
def Astar(start_node_pos,goal_node_pos,poly_points_updated, robot_dimension, robot_clearance):
...
robot_dimension = int(input('Enter robot dimension: '))
robot_clearance = int(input('Enter robot clearance: '))
...
if (s_flag):
    ...
    result,image = Astar(start_node_pos,goal_node_pos,poly_points_updated, robot_dimension, robot_clearance)
